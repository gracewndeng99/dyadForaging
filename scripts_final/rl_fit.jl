...
NSTEPS = 9 #9 STEPS to choose from between safety and danger
...
GENRULE = "peppgFull"
CHOICERULE = "econ"
ALPHARULE = "lrdecay" #"lrhist" #"lrflat" #"lrdecay"
PREDICTIONTYPE = "realPrediction"#"realPrediction"#"rollingAverage"#"learned"
...
function read_data(folder; prediction_type = PREDICTIONTYPE)
    df_group = CSV.read("../processed_data/parsed_group_$(folder).csv", DataFrame)
    df_idv = CSV.read("../processed_data/parsed_idv_$(folder).csv", DataFrame)

    # if only fit on subset
    # subset_ids = [249, 78, 50, 177, 214, 212, 305]
    # subset_ids = [137, 98, 99, 138, 159]
    # filter!(row -> row.subID in subset_ids, df_group)
    # filter!(row -> row.subID in subset_ids, df_idv)
    ...
    if prediction_type=="learned"
        ...
        pred_sim = CSV.read("../model_fits/rl_$(folder)/simulated_predictions.csv", DataFrame)
...
function update_V_safety(self_params, history, V_safety;
    genrule=GENRULE, choicerule=CHOICERULE, phase=1)

    curr_loc = Int(history[end][1])  # Get the current location (1-based index in Julia)
    attack = Int(history[end][2])

    if choicerule=="riskSensLearning"
        alpha = self_params[1:2]
        gamma = self_params[3]
    elseif choicerule=="riskSensGen"
        alpha = self_params[1]
        gamma = self_params[2:3]
    else 
    ...
    if attack == 1# If attacked
        ...
        if curr_loc != NSTEPS # Update all state values after that square
            ...
            elseif gen=="noGen"
                ...
                println("error: please input a valid genrulerule")
            end
        end
...
function get_location_util(loc, V_safety, params; 
    ...
    else # default to expected value
        Q = V_safety[i] * rewards[i] - (1 - V_safety[i]) * 10 
...
function get_sub_llh_idv(params, subdf::DataFrame; scorefunc=SCOREFUNC, beta=beta)
    ...
    for pt in [0, 1]
        ...
        for row in eachrow(g)
            ...
            if row.encounter!=1
                #update V_safety
                V_safety = update_V_safety(params, history, V_safety, phase=1)
            end
...
function get_Vsafety(bf_params, subdf)
    ...
    for pt in [0, 1]
        ...
        for row in eachrow(g)  # Start from round 2
            if row.encounter!=1
                V_safety = update_V_safety(bf_params, history, V_safety, phase=1)
...
function sim_choice_idv(bf_params, subdf; mytype="partial", beta=beta)
    ...
    for pt in [0, 1]
        ...
        history =[] # Initialize history as a list of tuples
        V_safety = collect(1.0 : -1/NSTEPS : 1/NSTEPS) 
...
function sim_sub_idv(bf_params_df, df_idv; mytype="partial")
    ...
    for s in unique(bf_params_df.subID)
        ...
        println("Sub: $s, Idv params: $bf_params")
...
function get_sub_llh_grp(params_to_optimize, sub_df, Vsafety_list, mymodel2; 
    ...
    if occursin("updateTheta", mymodel2)
        ...
        # if params_to_optimize[2] + theta_g < 0
        #     return 1e100
        # end
    ...
    for pt in [0, 1]
        ...
        for row in eachrow(g)
            ...
            if show_progress
                println("P(choice = $(row.playerStep)) = $score_t on trial $(row.trial)")
            end
...
function get_partner_pref(real_prediction, partner_history; 
    ...
    if prediction_type == "realPrediction"
        pred_partner_step = real_prediction != -1 ? real_prediction : (isempty(partner_history) ? 5 : partner_history[end])
...
function sim_choice_grp(bf_params, subdf, Vsafety_list, mymodel2; mytype="partial")
    ...
    for pt in [0, 1]
        ...
        for row in eachrow(g)  
            ...
            sim_choice = rand(Categorical(choice_prob))
            get!(sim_choices, pt, Dict{Int, Int}())[row.trial] = sim_choice
            ...
            if mytype=="partial"
                # Append what actually happened to history
                push!(self_history, (row.finalStep, row.attack))  
                get!(sim_attacks, pt, Dict{Int, Int}())[row.trial] = row.attack
            elseif mytype=="full"
                ...
                get!(sim_attacks, pt, Dict{Int, Int}())[row.trial] = sim_attack
            end                
            # println(V_safety)
        end
    end
...
function sim_sub_grp(bf_params_df, df_idv, df_grp, mymodel2; mytype="partial")
    ...
    for s in unique(bf_params_df.subID)
        ...
        bf_params = vec(Matrix(bf_params))
        println("Sub: $s, Group params: $bf_params")
...
function grid_search(sub_df_idv, sub_df_grp, bounds, grid_size, mymodel2)
    ...
    loss = zeros(length(combos))
    loss_idv = zeros(length(combos))
    loss_grp = zeros(length(combos))
    ...
    for i in 1:length(combos)
        ...
        # get Vsafety 
        Vsafety_list = get_Vsafety(par[1:3], sub_df_idv)
        alh_grp = get_sub_llh_grp(par, sub_df_grp, Vsafety_list, mymodel2)
        loss_grp[i] = alh_grp
        alh  = alh + alh_grp
        #add regularizatins? #maybe set almbda =2?
        # penalty = lambda_theta * ((par[2] - 1)/0.5)^2 + lambda_weight * ((par[4] - 0.5)/0.5)^2 + lambda_alpha * ((par[2] - 0.5)/0.5)^2
        # alh += penalty
        loss[i] = alh
    end

    best_idx = argmin(loss)
    best_v = loss[best_idx]
    best_idv = loss_idv[best_idx]
    best_grp = loss_grp[best_idx]
    best_params = combos[best_idx]
    # println(best_params)
    return best_params, best_v, best_idv, best_grp
end


#for efficiency
function generation_grid_search(curr_bounds, curr_grid_size, sub_df_idv, sub_df_grp, step_size, orig_bounds, mymodel2)
    best_params, best_v, _, _ = grid_search(sub_df_idv, sub_df_grp, curr_bounds, curr_grid_size, mymodel2)
...
function fit_all(df_idv, df_grp, bounds, mymodel2, output_fname; 
    ...
    subIDs = unique(df_idv.subID)

    # One slot per subject. Each thread writes only to its own index.
    results = Vector{NamedTuple{(:subID, :alpha, :theta, :gamma, :w, :nll_idv, :nll_grp),
                                Tuple{Int, Float64, Float64, Float64, Float64, Float64, Float64}}}(undef, length(subIDs))

    #Iterate over unique rooms (skipping the first one)
    Threads.@threads for i in eachindex(subIDs)
        ...
        for i = 1:2 #run 3 times
            new_bounds, new_grid_size = generation_grid_search(
                new_bounds, new_grid_size, sub_df_idv, sub_df_grp, step_size, bounds, mymodel2)
        end
        best_params, _, best_nll_idv, best_nll_grp = grid_search(sub_df_idv, sub_df_grp, new_bounds, new_grid_size*5/4, mymodel2)
        #round and log: step size is 0.01 or 0.02
        best_params = [round(i, digits=2) for i in best_params]
        println([best_params, best_nll_idv + best_nll_grp])
        ...
        results[i] = (
            ...
            gamma = best_params[3],
            w = best_params[4],
            nll_idv = best_nll_idv,
            nll_grp = best_nll_grp
        )
        #save to df
...
function recover_all(input_fname, mymodel2, df_idv, df_grp, fit_bounds; sim_type="full")
    # read simulated data
    ...
    #fit
    params_df = fit_all(sim_idv, sim_grp, fit_bounds, mymodel2, input_fname; step_size = 5, save=false)
    #save
...
function main(folder, mymodel2; recovery=false, mysimtype="full", k=10)
    ## read data
    df_idv, df_grp = read_data(folder; prediction_type=PREDICTIONTYPE)
    ...
    #define outputpath
    output_fname = "../model_fits/rl_$(folder)/$(mymodel)_$(mymodel2)"
    ...
    simulate_all(df_idv, df_grp, params_df, mymodel2, output_fname, mysimtype, k)

    # Model recovery
    if recovery
        recover_all(output_fname, mymodel2, df_idv, df_grp, bounds; sim_type=mysimtype)
    end
end
